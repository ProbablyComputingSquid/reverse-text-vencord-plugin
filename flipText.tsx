...
export function FlipIcon (props: IconProps) {
    return (
        <Icon
            {...props}
            className={classes(props.className, "vc-flip-icon")}
            viewBox="0 0 24 24"
        >
            <path opacity="0.5"
...
export default definePlugin({
    name: "flip Text",
    description: "flips all of your text",
    authors: [
        {
            name: "computingsquid",
    ...
    async start() {
        ...
        addButton("vc-flip", message => {
            return {
                ...
                onClick: async () => {
                    const flipedText = this.transformText(message.content);
                    updateMessage(message.channel_id, message.id, { content: flipedText });
                }
            };
        });
    },
    ...
    transformText(text: string) {
        ...
        const flipedParts = parts.reverse().map(part => {
            if (regex.test(part)) {
                return part;
            }
